...
bool SystemMonitorNode::start(int argc, char **argv)
{
	...
	logger->log_info("--- Subscribing to Resource Topics ---");
	for(std::size_t i = 0; i < resource_topics.size(); ++i)
	{
		logger->log_info("[" + std::to_string(i) + "] " + resource_topics.at(i));
	...
	logger->log_info("--- Subscribing to Heartbeat Topics ---");
	...
	{
		logger->log_info("[" + std::to_string(i) + "] " + resourceavailable_topics.at(i));
	...
	logger->log_info("--- Monitoring Tasks ---");
	...
	{//Center Footer
		//Generic Help Window
		{
			...
			if(select_task_mode == true)
			{
				...
				if(change_loglevel_mode == false)
				{
					tempstr1 = "\tl: Change Log Level for the selected task.";
				}
				else
				{
...
bool SystemMonitorNode::run_10hz()
{
	...
	//logger->log_debug(std::to_string(v));
	switch(v)
	{
		case ENTER_KEY:
			enter_pressed = true;
