...
TEST(SystemMonitor, TestSystemMonitor) {
    ros::NodeHandle nh("~");
    Logger* logger = new Logger("DEBUG", "test_systemMonitor");

    std::string heartbeat_topic = "/test/system_monitor/heartbeat";
    ...
    usleep(1.0 * 1000000.0);  // Wait for System Monitor to Start.
...
int main(int argc, char** argv) {
    ...
    ros::init(argc, argv, "test_systemMonitor");
