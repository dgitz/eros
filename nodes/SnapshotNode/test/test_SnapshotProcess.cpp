...
TEST(BasicTest, TestOperation_Master) {
    ...
    EXPECT_TRUE(tester->get_systemsnapshot_state() == SnapshotProcess::SnapshotState::STARTED);
    diag_list = tester->createnew_snapshot();
    EXPECT_TRUE(tester->get_systemsnapshot_state() == SnapshotProcess::SnapshotState::NOTRUNNING);
